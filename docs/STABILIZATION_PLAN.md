...
- ✅ **Guard added (2026-06-18):** `audit_core_writers.py --ci` runs in `ci-quality-gate.yml`. The 18 sites are baselined; CI now blocks any **NEW** file from writing a core table directly. Migrating the 18 onto `DrugWriter().update_fields()` / `CompanyWriter` / `CatalystWriter` is the remaining Phase-2 work (some touch risky columns → needs care + Kyle for `company_id`/`stage`).
