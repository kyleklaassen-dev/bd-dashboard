...
- ✅ **Field-update bypasses MIGRATED (2026-06-18):** all 17 real direct field-writes (16 `drugs`, 1 `companies`) across 9 files now route through `update_drug()` / `update_company()` (`src/meridian/database/__init__.py` → DrugWriter/CompanyWriter `update_fields`). The 1 remaining audit match is a false positive (the `sb_upsert('catalysts')` text in the `_catalyst_upsert` docstring). **Real direct-write debt to core tables is now ZERO** (audit `--strict` shows only the docstring). Verified: writer suites 23/23 green, all migrated modules import, byte-compile clean. `update_fields` is governance-checked, so these writes now get the same validation as the create paths.
- ⚠️ **Missing artifacts (2026-06-18):** `migrations/PROPOSED_drugwriter_enforcement.sql` and `migrations/PROPOSED_drop_dead_tables.sql` are referenced here but **are not in the repo** (no `PROPOSED_*` files exist). The DB-trigger enforcement and dead-table drops need to be re-authored before they can be applied.
- ⬜ Apply enforcement (re-author `PROPOSED_drugwriter_enforcement.sql`) — needs Kyle + watch window. Extend the trigger pattern to companies/catalysts.
