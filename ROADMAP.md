...
5. ✅ **`index.html` Phase 4** decomposition (§4) — **DONE 2026-06-18**: index.html **34,847 → 7,124 lines (−79.6%)**.
   ...
   14 tab/feature modules) and all top-level CSS to `assets/css/` (12 `<style>`→`<link>`, same-position = cascade-preserved).
   Every step byte-identical + preview-verified (HTTP 200 + render + globals/computed-styles + 0 console errors); PRs #6–#12.
   Remaining = small inline glue + the in-JS `<style>` string in app.js (low value). **§A.2 (UI/logic separation — moving
   `_resolveStage`/`_score`/`_dedup` server-side) is the deeper, separate correctness goal** still open (see §A.2 below).
