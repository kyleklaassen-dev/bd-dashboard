...
Output:
  data/prompt_improvements.json — structured improvement guidance
  data/field_quality_report.json — which fields drift most often
"""
...
def load_training_pairs(date: Optional[str] = None) -> list:
    ...
    if date:
        path = os.path.join(_DATA, f"training_pairs_{date}.jsonl")
    else:
        files = sorted(glob.glob(os.path.join(_DATA, "training_pairs_*.jsonl")), reverse=True)
...
def load_drift_report(date: Optional[str] = None) -> dict:
    ...
    if date:
        path = os.path.join(_DATA, f"drift_report_{date}.json")
    else:
        files = sorted(glob.glob(os.path.join(_DATA, "drift_report_*.json")), reverse=True)
        path = files[0] if files else None
...
def inject_into_enrichment(prompt_block: str):
    ...
    hints_path = os.path.join(_REPO, "data", "enrichment_prompt_hints.md")
...
def main():
    ...
    # Write outputs
    improvements_path = os.path.join(_DATA, "prompt_improvements.json")
    ...
    quality_path = os.path.join(_DATA, "field_quality_report.json")
