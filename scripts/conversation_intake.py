...
SVC_KEY   = open(os.path.join(WORKSPACE, '.supabase_service_key')).read().strip()
ANT_KEY   = open(os.path.join(WORKSPACE, '.anthropic_api_key')).read().strip()
...
def store_facts(facts: list[dict], source: str = 'conversation') -> int:
    ...
    inserted = 0
    for row in rows:
        r = requests.post(f'{BASE}/conversation_intelligence_intake', headers=H_SVC, json=row)
...
def show_pending():
    ...
    r = requests.get(
        f'{BASE}/conversation_intelligence_intake?review_status=eq.pending&order=auto_confidence.desc',
        headers=H_SVC
    )
    items = r.json()
...
def promote_item(item_id: int):
    ...
    r = requests.get(
        f'{BASE}/conversation_intelligence_intake?id=eq.{item_id}',
        headers=H_SVC
    )
    items = r.json()
    if not items:
    ...
    if r2.status_code in (200, 201, 204, 200):
