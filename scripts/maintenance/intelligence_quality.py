...
def collect():
    ...
    # 3 + 4 + 5. completeness. drugs.completeness_tier is the per-drug score, written by
    # `completeness-scoring.yml` (research_intelligence --area all → upsert_research_queue stamps
    # each drug). research_queue holds the per-entity×area companion. An "untiered" drug = one the
    # scoring workflow hasn't covered yet (e.g. not in drug_areas, or added since the last run).
